...
#define immIU() do { *imm = ZEXT(BITS(i, 31, 20), 12); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = ZEXT(Mr(src1 + imm, 1), 8));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = ZEXT(Mr(src1 + imm, 2), 16));
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhsu , R, R(rd) = ((int64_t)(int32_t)src1 * (uint64_t)src2) >> 32);
